...
const USMSideBar = ({Direction, ComponentSize}) => {
  ...
  const items = [
    ...
    isRole([ROLE.ADMIN]) && getItem('Cài đặt', 'setting', <SettingOutlined />, [
      ...
      getItem(<Link to={URL.SETTING_SHIFT} style={{display:"flex", alignItems: "center", justifyContent: "space-between"}}><span>Ca làm việc</span><ScheduleOutlined /></Link>, 'shift-setting'),
      getItem(<Link to={URL.SETTING_CALENDAR} style={{display:"flex", alignItems: "center", justifyContent: "space-between"}}><span>Lịch làm việc</span><ReconciliationOutlined /></Link>, 'calendar-setting'),
    ]),
